...
if (isset($_SESSION['user']['role']) == false) {
    ...
    if (!empty($_FILES['pic1']['name'])) {
        ...
        $path1 = null; // Или установите другое значение по умолчанию
    }
    ...
    if (!empty($_FILES['pic2']['name'])) {
        ...
        $path2 = null; // Или установите другое значение по умолчанию
    }


    $addpost = "INSERT INTO `news`(`id`,`date`,`header`,`pic`,`link`) VALUES('$newid', '$date', '$name', '$path', '$link')";
    $add = mysqli_query($db, $addpost);
    $queryaddnews = "INSERT INTO `newspage`(`vstavka`, `header`, `text`, `pic1`, `pic2`,`id`) VALUES('$vst','$name','$text','$path1','$path2','$newid')";
